...
knitr::include_graphics(path = here::here("docs/model-pres_June2025.png"))
```
...
```{r nys-bb-bathymetry, eval = TRUE, echo = TRUE}
...
The chlorophyll a data currently covers 1-month past the final trawl data of 2024-08-14 (i.e., 2024-09-14). For the purposes of this initial exercise, we would treat this next month for forecasting purposes.
...
Copernicus also provides reanalysis of low and mid-trophic level biomass [here](https://data.marine.copernicus.eu/product/GLOBAL_MULTIYEAR_BGC_001_033/download?dataset=cmems_mod_glo_bgc_my_0.083deg-lmtl_P1D-i_202511). These are measures of zooplankton biomass for the study area.
...
```{r lmtl-forecast-plot, echo=TRUE, eval=TRUE}
...
The Atlantic multidecadal oscillation SST monthly index can be downloaded from [NOAA/NCEI](https://www.ncei.noaa.gov/pub/data/cmb/ersst/v5/index/ersst.v5.amo.dat). This product is a high level variable and doesn't necessarily operate at a temporal scale that makes sense for short-term forecasting. Here were have the index for the monthly AMO sea surface temperature anomaly.
...
The North Atlantic Oscillation index is an index of broader weather patterns and is estimate from the pressure height contrasts from the North-South regions compared to the long-term climatological data. The monthly values can be accessed via [NCEP](https://www.cpc.ncep.noaa.gov/products/precip/CWlink/pna/norm.nao.monthly.b5001.current.ascii).
...
readRDS(here('data/environmental/nao.rds')) %>% 
  dplyr::mutate(future = ifelse(as.Date(date) <= as.Date("2024-08-14"), FALSE, TRUE)) %>%
  ggplot()+
  geom_point(aes(x = date, y = nao, color = future))+
  geom_line(aes(x = date, y = nao, color = future))+
  scale_color_manual(values = c('black','red'))+
...
```{r NAO-forecast}
nao_meas = readRDS(here('data/environmental/nao.rds')) %>% 
  dplyr::filter(date <= as.Date("2024-08-01")) %>% 
  dplyr::mutate(nao_sd = NA,
                future = FALSE)
  
nao_pred = read.csv(here('data/gfs/outputs/nao_gfs_weekly.csv'), header = TRUE) %>% 
  dplyr::select(date = 'window_end', nao = 'nao_mean', nao_sd) %>% 
    dplyr::mutate(date = as.Date(date, format = "%Y-%m-%d"),
                  future = TRUE)

nao_fore = bind_rows(nao_meas, nao_pred)

nao_fore %>% 
  ggplot()+
  geom_point(aes(x = date, y = nao, color = future))+
  geom_line(aes(x = date, y = nao, color = future))+
  geom_ribbon(aes(x = date, ymin = nao-nao_sd, ymax = nao+nao_sd), color = 'red', fill = 'red', alpha = 0.5)+
  scale_color_manual(values = c('black','red'))+
  scale_y_continuous(name = 'North Atlantic Oscillation (NAO) Index',
                     limits = c(NA,NA))+
...
```{r lmtl-timeseries, fig.align='center', out.width='70%'}
knitr::include_graphics(here('ignore/zooc_animation.gif'))
